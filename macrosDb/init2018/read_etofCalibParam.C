...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet *DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofCalibParam" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofCalibParam *dataset = 0;
    dataset = ( St_etofCalibParam* ) DB->Find( "etofCalibParam" );
    ...
    if( dataset ) {
        ...
        etofCalibParam_st *table = dataset->GetTable();
        for( int i = 0; i < rows; i++ ) {
            std::cout << "get4TotBinWidthNs: " << table[ i ].get4TotBinWidthNs << "   minDigisInSlewBin: " << (int) table[ i ].minDigisInSlewBin << std::endl;
        }
    }
    else {
