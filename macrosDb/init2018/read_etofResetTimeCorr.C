...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet *DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofResetTimeCorr" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofResetTimeCorr *dataset = 0;
    dataset = ( St_etofResetTimeCorr* ) DB->Find( "etofResetTimeCorr" );
    ...
    if( dataset ) {
        ...
        etofResetTimeCorr_st *table = dataset->GetTable();
        for( int i = 0; i < rows; i++ ) {
            std::cout << "reset time offset correction: " << table[ i ].resetTimeOffset << std::endl;
        }
    }
    else {
