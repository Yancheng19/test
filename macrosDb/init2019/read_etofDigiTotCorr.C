...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet* DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofDigiTotCorr" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofDigiTotCorr* dataset = 0;
    dataset = ( St_etofDigiTotCorr* ) DB->Find( "etofDigiTotCorr" );
    ...
    if( dataset ) {
        ...
        etofDigiTotCorr_st* table = dataset->GetTable();
        for( int i=0; i<rows; i++ ) {
            ...
            for( int j=0; j<6912; j++ ) {      
                ...
                std::cout << "channel " << j << ": " << sector << "  " << module << "  " << counter << "  " << strip << "  " << side << " --> time correction = " << table[ i ].totCorr[ j ] << std::endl;
            }
        }
    }
    else {
