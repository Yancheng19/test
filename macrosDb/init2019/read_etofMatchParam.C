...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet *DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofMatchParam" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofMatchParam* dataset = 0;
    dataset = ( St_etofMatchParam* ) DB->Find( "etofMatchParam" );
    ...
    if( dataset ) {
        ...
        etofMatchParam_st *table = dataset->GetTable();
        for( int i = 0; i < rows; i++ ) {
            std::cout << "match radius: " << table[ i ].matchRadius << "    track cut nHitsFit: " << ( int ) table[ i ].trackCutNHitsFit << std::endl;
            std::cout << "track cut nHits ratio: " << table[ i ].trackCutNHitsRatio << "    track cut low pt: " << table[ i ].trackCutLowPt << std::endl;
        }
    }
    else {
