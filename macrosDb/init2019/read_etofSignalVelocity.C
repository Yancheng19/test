...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet *DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofSignalVelocity" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofSignalVelocity *dataset = 0;
    dataset = ( St_etofSignalVelocity* ) DB->Find( "etofSignalVelocity" );
    ...
    if( dataset ) {
        ...
        etofSignalVelocity_st *table = dataset->GetTable();
        for( int i = 0; i < rows; i++ ) {
            ...
            for( int j=0; j<108; j++ ) {
                ...
                std::cout << "signalVelocity (" << j << ") of ( sector, module, counter ) " <<  sector << "  " << module << "  " << counter;
                std::cout << "    ---> " << table[ i ].signalVelocity[ j ] << std::endl;
            }
        }
    }
    else {
