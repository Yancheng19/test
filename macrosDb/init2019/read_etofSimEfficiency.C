...
//    gSystem->Load("libmysqlclient");
    ...
    TDataSet *DB = 0;
    DB = dbMk->GetDataBase( "Calibrations/etof/etofSimEfficiency" );
    if( !DB ) {
        std::cout << "ERROR: no table found in db, or malformed local db config" << std::endl;
        return;
    }

    St_etofSimEfficiency *dataset = 0;
    dataset = ( St_etofSimEfficiency* ) DB->Find( "etofSimEfficiency" );
    ...
    if( dataset ) {
        ...
        etofSimEfficiency_st *table = dataset->GetTable();
        for( int i = 0; i < rows; i++ ) {
            ...
            for( int j=0; j<108; j++ ) {
                ...
                std::cout << "efficiency (" << j << ") of ( sector, module, counter ) " <<  sector << "  " << module << "  " << counter;
                std::cout << "    ---> " << table[ i ].efficiency[ j ] << std::endl;
            }
        }
    }
    else {
