...
//    gSystem->Load("libmysqlclient");
    ...
    StDbTable*      dbtable   = node->addDbTable( "etofHitParam" );
    TString         storeTime = "2018-12-10 00:00:00"; // beginTime timestamp in MySQL format: "YYYY-MM-DD HH:mm:ss"
    mgr->setStoreTime( storeTime.Data() );

    // Create your c-struct
    etofHitParam_st table;
    
    // Fill structure with data 
    table.maxLocalY = 15.;
    table.clusterMergeRadius = 1.;

    std::cout << "maximum localY: " << table.maxLocalY << "    cluster merge radius: " << table.clusterMergeRadius << std::endl;
